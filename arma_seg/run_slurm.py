...
with open(f'{args.base_dir}/output/{args.env}/now.txt', "w") as nowfile,\
    ...
    for idx, ( lr, wd,lr_scheme) in enumerate(params):
        now = datetime.now()
        ...
        cmd = 'python main_normal.py --exp_name ' +'output/'+ str(idx) +' --batch_size 32 --workers 16 \
...
with open(slurm_script_path, 'w') as slurmfile:
    ...
    slurmfile.write("source /vulcan/scratch/shishira/miniconda3/bin/activate \n")

    slurmfile.write("mkdir -p /scratch0/shishira/ \n")

    slurmfile.write("/vulcanscratch/shishira/msrsync pascal_voc /scratch0/shishira/ -p 20 -P \n")
