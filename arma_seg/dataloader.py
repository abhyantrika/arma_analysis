...
class pascalVOCLoader(data.Dataset):
	...
	def transform(self, img, lbl):
		...
		img = self.tf(img)
		lbl = torch.from_numpy(np.array(lbl,dtype=np.int32)).long()

		lbl[lbl > 249] = 0

		if torch.max(torch.unique(lbl)) > 20:
			pdb.set_trace()
	...
	def setup_annotations(self):
		...
		if len(pre_encoded) != expected:
			...
			for ii in tqdm(sbd_train_list):
				...
				if os.path.exists(pjoin(target_path,ii+'.png')):
					continue
	...
	def get_loaders(self):
		...
		self.train_dataset = pascalVOCLoader(self.args,self.root,sbd_path=self.sbd_path,split='train_aug',augmentations=self.augmentations,\
			img_size=self.img_size)
		self.val_dataset = pascalVOCLoader(self.args,self.root,sbd_path=self.sbd_path,split='val',img_size=self.img_size)
