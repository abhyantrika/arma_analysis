...
for eps in epsilons:
	for att in attacks:
		...
		# baseline_attack_cmd = 'python attack_block_freq.py --model_path ' + baseline_path + ' --model_arch '+args.model_arch+ \
		# ' --eps '+str(eps) + ' --batch_size 512 --baseline --save_folder ' + \
		# baseline_save_folder  + ' --attack '+str(att) +' --iters 10'

		

		arma_attack_cmd = 'python attack_block_freq.py --model_path ' + arma_path + ' --model_arch ' + args.model_arch +\
